...
run(function()
    -- king ben 82: king ben (multi-target aura) + Closest Silent Aim 2 (silent rotation + hitreg) combined
    local KingBen82
    local Targets
    local Speed
    local Range
    ...
    KingBen82 = vape.Categories.Blatant:CreateModule({
        Name = 'king ben 82',
        Function = function(callback)
            if callback then
                local lastent, lastfound = nil, 0
                ...
                repeat
                    ...
                    if sword then
                        ...
                        local ents = entitylib.AllPosition({
                            ...
                            Players = Targets.Players.Enabled,
                            NPCs = Targets.NPCs.Enabled,
                            Limit = math.max(MaxTargets.Value, 1),
                            Sort = sortmethods[Mode.Value] or sortmethods.Distance,
                        })
                        ...
                        if #ents > 0 then
                            ...
                            local primary = ents[1]
                            local localfacing = (inputService.KeyboardEnabled and gameCamera or entitylib.character.RootPart).CFrame.LookVector * Vector3.new(1, 0, 1)
                            local pdelta = (primary.RootPart.Position - localPosition)
                            local pflat = pdelta * Vector3.new(1, 0, 1)
                            local facingdot = pflat.Magnitude > 0 and localfacing.Magnitude > 0 and (localfacing / localfacing.Magnitude):Dot(pflat / pflat.Magnitude) or 0
                            if facingdot < math.cos(math.rad(Angle.Value) / 2) then
                                lastfound = 0
                                continue
                            end

                            if lastent ~= primary then
                                foundat = tick()
                            end
                            ...
                            for _, ent in ents do
                                ...
                                local nh = nextHit[key] or 0
                                local delta = (ent.RootPart.Position - localPosition)

                                if delta.Magnitude > bedwars.CombatConstant.RAYCAST_SWORD_CHARACTER_DISTANCE then
                                    if current >= nh then
                                        nextHit[key] = current + delay
                                        pcall(SwingMissRemote.FireServer, SwingMissRemote, {chargeRatio = 0, weapon = sword.tool})
                                    end
                                    continue
                                end
        ...
        Tooltip = 'king ben + Closest Silent Aim combined: silent multi-target aura with hitreg',
    })

    Targets = KingBen82:CreateTargets({Players = true, NPCs = true})
    Speed = KingBen82:CreateSlider({
        Name = 'Aim speed',
        Min = 1,
        Max = 10,
        Default = 6,
    ...
    Angle = KingBen82:CreateSlider({
        Name = 'Max angle',
        Min = 1,
        Max = 360,
        Default = 180,
    })
    MaxTargets = KingBen82:CreateSlider({
        Name = 'Max targets',
        Min = 1,
        Max = 5,
        Default = 3,
    ...
    LegitAura = KingBen82:CreateToggle({Name = 'Swing only'})
    SilentAim = KingBen82:CreateToggle({
        Name = 'Silent Aim',
        Default = true,
    })
